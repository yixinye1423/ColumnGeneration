...
def getData(k):
	...
	for h in range(len(matrix_k)):
		...
		singlepn_LO2 = [3650*pn_LO2*sum(numpy.multiply(numpy.asarray(isFailure), numpy.multiply(numpy.asarray(pi), numpy.multiply(numpy.asarray(diag), diag_exp_LO2[n]))))
		 for n in range(len(V_LO2))]
		singlepn_LN2 = [3650*pn_LN2*sum(numpy.multiply(numpy.asarray(isFailure), numpy.multiply(numpy.asarray(pi), numpy.multiply(numpy.asarray(diag), diag_exp_LN2[n])))) 
		for n in range(len(V_LN2))]
...
def combine(dist):#calculate fInv_LO2, fInv_LN2
	...
	avaiInd = sparse.csr_matrix([1 if sum(tup)==0 else 0 for tup in itertools.product(*fails[::-1]) ])#available scenarios
	pi_hat = multiply(pis)
	print(pi_hat.shape)
...
def combine_simple(dist):
	...
	for n in range(N):
		fInv_LO2[n] = sum(dist[k]['singlepn']['LO2'][n] for k in range(len(dist)))
		fInv_LN2[n] = sum(dist[k]['singlepn']['LN2'][n] for k in range(len(dist)))
	return (fInv_LO2, fInv_LN2)
...
for h_bar in range(len(fInv1)):
	...
	forPlot3.append(fInv_LO22[(1,h_bar)]/fInv_LO21[(1,h_bar)])
	forPlot4.append(fInv_LO21[1,h_bar]+c_hat_hbar[h_bar]+c_LO2[1])

#plt.plot(list(range(len(forPlot4))), forPlot4)
#print(forPlot1)
#print(forPlot2)
#print(forPlot3)
...
for n in range(len(V_LO2)):
	...
	for h_bar in range(len(fInv1)):
		forPlot5[n].append(fInv_LO21[n,h_bar]+c_hat_hbar[h_bar]+c_LO2[n]+fInv_LN21[n,h_bar]+c_LN2[n])
for n in range(len(V_LO2)):
