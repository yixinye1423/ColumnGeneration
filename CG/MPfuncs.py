...
def listCost(cap, designs):
	Cap = dict()
	for k in range(len(cap)):
		for h in range(len(designs[k])):
			cost = sum(cap[k][j] for j in designs[k][h])
			Cap[(k,h)]=cost
	return Cap
...
def pseudoMat(k, design_k, options, failureModes, parameters):
	...
	for design_k_h in design_k:
		matrix_k_h, states_k_h = generateTMatrix(k, design_k_h, parameters[k], options, failureModes)#list of dictionaries
...
def MPdataInit(unitNum, options, failureModes, parameters, cap):
	mstDat = dict()
	mstDat['N'] = {None:list(range(len(V_LO2)))}#storage options
	mstDat['c_LO2'] = {n: c_LO2[n] for n in range(len(c_LO2))}
	mstDat['c_LN2'] = {n: c_LN2[n] for n in range(len(c_LN2))}

	mstDat['K'] = {None:list(range(len(unitNum)))}#stage
	designs = [None]*len(unitNum)#initiate design space
	...
	hs = [list(range(len(designs[k]))) for k in range(len(designs))]#number of designs for each stage
	mstDat['H'] = {None:list(range(max([len(hs[k]) for k in range(len(hs))])))}#full length design space = max number of designs	
	mstDat['c_hat'] = listCost(cap, designs)#cost for currently included designs

	list2d = [[(k,h) for h in hs[k]] for k in range(len(hs))]
	mstDat['KH'] = {None:[val for sublist in list2d for val in sublist]}#valid stage-design pairs
	hbars = list(itertools.product(*hs))
	mstDat['H_bar'] = {None:list(range(len(hbars)))}#currently included system designs
	list2d = [[(k,hbars[i][k],i) for k in range(len(hbars[i]))] for i in range(len(hbars))]
	mstDat['D'] = {None:[val for sublist in list2d for val in sublist]}#currently included stage-design-system design pairs
	
	global designs01
	designs01 = copy.deepcopy(designs)
	print(designs01)
	for k in range(len(designs)):
		for h in range(len(designs[k])):
			designs01[k][h] = [0]*unitNum[k]
			for j in range(unitNum[k]):
				designs01[k][h][j] = 1 if j in designs[k][h] else 0

	global stageData 
	stageData = list()
	for k in range(len(unitNum)):
		data = getData(k, designs01[k], options, failureModes, parameters)#contains (pi, diag, isFailure) for each design in each stage
		stageData.append(data)

	fInv = list()
	for comb in itertools.product(*stageData):
		fInv.append(combine(comb))#include kronecker sum and products; format: [(fInv_LO2, fInv_LN2)]
	fInv_LO2 = dict()
	fInv_LN2 = dict()
